...
public:
    ...
    std::vector<int64_t> integral;   // summed-area table slike -> O(1) f_bar po prozoru
    int img_w, img_h, tmp_w, tmp_h;
    uint64_t img_addr, tmp_addr;
    int      template_mean;
    uint32_t hw_status;
    bool     img_dirty;              // CPU upisao REG_IMG_ADDR -> nov segment, ponovo učitaj
...
private:
    ...
    void build_integral_image();     // gradi SAT slike jednom po segmentu
