...
architecture rtl of ncc_core is
    ...
    constant DM_W : integer := 18;
    constant DN_W : integer := 83;
...
begin
    ...
    begin
        ...
        case state_reg is
            ...
            when S_L_U_A =>
                sat_rd_addr <= (v_reg + tmp_h_reg) * SAT_W + (u_reg + tmp_w_reg);
                state_next <= S_L_U_B;

            when S_L_U_B =>
                sum_f_partial_next <= resize(sat_rd_data, 32);
                sat_rd_addr <= v_reg * SAT_W + (u_reg + tmp_w_reg);
                state_next <= S_L_U_C;

            when S_L_U_C =>
                sum_f_partial_next <= sum_f_partial_reg - resize(sat_rd_data, 32);
                sat_rd_addr <= (v_reg + tmp_h_reg) * SAT_W + u_reg;
                state_next <= S_L_U_D;

            when S_L_U_D =>
                sum_f_partial_next <= sum_f_partial_reg - resize(sat_rd_data, 32);
                sat_rd_addr <= v_reg * SAT_W + u_reg;
                state_next <= S_L_U_E;
            ...
            when S_L_YX_FILL =>
                img_addr_o   <= v_reg * img_w_reg + u_reg;
                templ_addr_o <= 0;
                mac_v_next   <= '0';
