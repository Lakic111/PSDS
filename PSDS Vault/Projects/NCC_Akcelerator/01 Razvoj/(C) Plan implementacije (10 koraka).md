...
Zynq PS + Block Memory Generator + BRAM Controller + DMA + IP jezgro (×2, za ncc0 i
ncc1). Adresna mapa iz `common.hpp` se koristi direktno. Napomena: `Vezba 8-9` NE
pokriva ovaj korak (samo pakovanje IP-a) — kad dođe na red, tražiti dodatni izvor.

**Status:** nije započeto.

## Korak 8 — Analiza integrisanog sistema [5 bodova]

- 8a-8d isto kao korak 5, ali za integrisani sistem
- 8e **Poređenje sa predviđanjima iz "Projektovanje elektronskih uređaja na
  sistemskom nivou"** — odstupanje throughput-a i frekvencije ≤ 20%. **Referentne
  brojke (potvrđene, iz ESL dokumentacije):** ukupno vreme obrade `board2.txt` (32
  popunjenih polja) = **3.667 s** (2 NCC bloka + sve optimizacije, 10.8× ubrzanje u
  odnosu na 1 NCC bez optimizacija = 39.79 s). Ovo je glavna throughput referenca —
  naš merene vreme na realnoj ploči treba da bude u okviru ±20% od 3.667 s (uz isti
  test fajl/sliku). Frekvencija: cilj 100 MHz (10 ns), HLS procena 7.3 ns perioda.
  Detalji: `00 Pregled/(C) ESL dokumentacija - izvod.md`.
