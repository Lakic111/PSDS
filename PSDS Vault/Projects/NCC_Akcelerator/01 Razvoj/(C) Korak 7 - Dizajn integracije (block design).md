...
| 5 | **`board_part digilentinc.com:zybo:part0:2.0`** | **POTVRĐENO 2026-08-26** — ploča ima VGA + 1× HDMI → originalni Zybo. Do tada je stajalo `zybo-z7-10:part0:1.2` kao pretpostavka, i sve brojke u ovom dokumentu su merene sa TIM presetom. Part `xc7z010clg400-1` je isti; PS preset nije — tajming se mora ponovo potvrditi. |
...
> `board_part` je **`digilentinc.com:zybo:part0:2.0`** — vrednost iz tabele ispod je bila
> tačna. Segment verzije je `<file_version>` iz `board.xml` (B.3 → `1.0`, B.4 → `2.0`),
> ne ime direktorijuma i ne `schema_version`; `2.0` je dakle revizija B.4. Merodavan
> izvor je `get_board_parts`. Odeljak ispod je istorijski zapis nerazrešenog pitanja.
